...
Gui::Gui(QWidget *parent) : QMainWindow(parent) {
    currentPlotIndex = 0; // Initialize to the first plot
    numberOfPlots = 7; // Set to the total number of indicator plots 
    setupUI();
    //displayHtmlContent();
}
...
void Gui::setupUI() {
    ...
    prevButton = new QPushButton("Previous Indicator", this);
    nextButton = new QPushButton("Next Indicator", this);
    
        
    // Connect the buttons to slots
    connect(prevButton, &QPushButton::clicked, this, &Gui::onPrevClicked);
    connect(nextButton, &QPushButton::clicked, this, &Gui::onNextClicked);
...
void Gui::onPrevClicked() {
    if (currentPlotIndex > 0) {
        currentPlotIndex--;
        displayHtmlContent(currentPlotIndex);
        plotView->show();
    }
}

void Gui::onNextClicked() {
    if (currentPlotIndex < (numberOfPlots - 1)) { // numberOfPlots is the total number of plots
        currentPlotIndex++;
        displayHtmlContent(currentPlotIndex);
        plotView->show();
    }
...
void Gui::onButtonClicked() {
    ...
    const size_t MAX_ROWS = 1000; // Maximum rows to display

    og_df.cropToLastNRows(MAX_ROWS);
...
// }
void Gui::displayHtmlContent(int index) {
    std::string user_path = getPath();
    std::vector<std::string> htmls = {"true_vs_pred.html", "dual_thrust.html", "bollinger_bands.html", "heikin_ashi.html", "awesome.html", "macd.html", "rsi.html"};

    if (index < 0 || index >= htmls.size()) return; // Index out of bounds check

    QFile file(QString::fromStdString(user_path + "csc3380-fall-2023-project-group-3/data/" + htmls[index]));
    if (file.open(QFile::ReadOnly | QFile::Text)) {
        QString htmlContent = file.readAll();
        plotView->setHtml(htmlContent);
    } else {
        plotView->setHtml("<html><body><h1>Error: Unable to open plot!</h1></body></html>");
    }
}
